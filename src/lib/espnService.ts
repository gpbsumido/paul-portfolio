...
const safeFetchWithRetry = async (url: string, options: RequestInit, timeout: number, retries = 2) => {
    try {
...
export const getLeagueInfo = async () => {
    const url = new URL(`${ESPN_FANTASY_API.BASE_URL}/seasons/${ESPN_FANTASY_API.SEASON}/segments/${ESPN_FANTASY_API.SEGMENT}/leagues/${ESPN_FANTASY_API.LEAGUE_ID}`);
...
export const getHistoricalLeagueInfo = async (year: string) => {
    const url = new URL(`${ESPN_FANTASY_API.BASE_URL}/seasons/${year}/segments/${ESPN_FANTASY_API.SEGMENT}/leagues/${ESPN_FANTASY_API.LEAGUE_ID}`);
    url.searchParams.append("view", "mTeam");
    ...
    if (!response.ok) {
        throw new Error(`Failed to fetch historical league info: ${response.status} ${response.statusText}`);
    }
